...
- From `BACKLOG.md`: Continue **P1-7** (`elt schedule run` rejects its own plan — `CONFIG_VALIDATION_FAILED`, exit 2) → schedule-plan schema drift; reconcile [scheduler.py](src/elt_pipeline/shared/scheduler.py) with the example plan + placeholder-resolution contract. Verify with `uv run pytest tests/test_cli.py -k schedule -q`. Then **P1-8** / **P2-9**. Workable in a cold session; does **not** need S-0.
...
- **Done:** P0-1, P0-2 (+session.py URI bug), P0-3, P0-4, P1-5, **P1-6**, **S-1**, **S-2**, P2-10. **Per-file verified:** `test_normalize_pipeline` 9/9; `test_iceberg_catalog_config` 34/34; `test_sql_models` **25/25** (iceberg-off); `test_staging_swap` 26/26; `test_iceberg_parity_and_audit` **25/25**.
...
- **Captured:** 2026-08-18. P0-1..P0-4, P1-5, the session.py URI fix, `CLAUDE.md`, and this doc are committed in **`d8fb234` (elt104)**; the `ELT_PIPELINE_TEST_SPARK_ICEBERG` conftest knob landed in **elt105/elt106**; the **S-1** append-fixture fix in **elt107**. The **S-2** fix (multi-level dynamic partition overwrite in `_staging_swap.py` + regression tests) is committed on top as **elt108**. The **P1-6** fix (`_build_serving_endpoint` early-out guard in `cli.py`) is in the working tree, uncommitted, at this session's end. Re-stamp whenever counts change.
