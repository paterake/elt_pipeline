...
**Maturity at a glance:** [Capability Maturity Matrix](docs/CAPABILITY_MATURITY_MATRIX.md) — every feature is classified 🟢 Production / 🟠 Demo. **No pre-scoped ⏳ roadmap rows remain.** Gate: 875 passed / 0 failed / 28 skipped.
...
- **Semantic Metrics Layer (GAP-4):** Thin manifest-layer package at `<package>/metrics/<domain>/<metric>/metric.yaml` — declares `query_ref: stage.domain.model.column` pointer + `aggregation` + `dimensions[]` + `filters[]` + `required_role: DataClassification`. Compile-time `--with-sql-refs` gate validates pointer against SqlColumnSpec governance (fail-fast before JVM). Three resolution modes at runtime: (a) Iceberg materialize via Spark executor, (b) Trino SECURITY DEFINER VIEW (G-6 classification → SECURITY DEFINER wrapper), (c) Prometheus gauge export via G-2 adapter. Cross-mode `generated_sql_hash` consistency guardrail; mismatches → METRIC_MODE_INCONSISTENT. Writes `metric_audit.jsonl` lines to `runs/<run_id>/metrics/`.
