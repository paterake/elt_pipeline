...
3. **What is not built yet (⏳ Roadmap):** DBFS / HDFS storage, real JDBC DB sources, real Kafka broker, PII masking/retention/erasure, container deployment artifacts. All are well-scoped adds behind existing seams (or explicit roadmap items) and tracked when pulled forward.
