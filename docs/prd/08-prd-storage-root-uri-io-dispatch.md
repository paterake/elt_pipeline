...
The routing decision is `detect_scheme(path)` (a lightweight `startswith`/prefix parse against a closed `StorageScheme` enum — no third-party URI parser, no `urllib.parse`, no netloc/path split). The parsed scheme is the sole dispatch key into a `_BACKEND_REGISTRY` dict mapping `StorageScheme → StorageBackend`.

**Canonical implementation pattern (B-6, shipped):**
1. **`StorageBackend` Protocol** — a `@runtime_checkable` Protocol declaring 18 leaf IO ops (`path_exists`, `path_read_bytes`, `path_write_text`, …) plus one staging-swap atomic op `staging_swap_atomic(*, staging_path, target_path, mode: SwapMode)`.
2. **Per-scheme backend classes** — `LocalBackend` (pathlib/os/shutil POSIX impl), `S3Backend` (boto3 impl). GCSBackend / ADLSBackend / etc. ship later.
3. **Registry + accessor** — `_BACKEND_REGISTRY: dict[StorageScheme, StorageBackend]` singleton, `get_backend(path: str) -> StorageBackend` resolves `detect_scheme(path)` → registry lookup, `register_backend(scheme, backend)` for explicit registration of new schemes at import time.
4. **`path_utils` public functions become one-line dispatchers** — every public `path_utils.path_*` function calls `_get_backend(root).path_*(...)`, preserving full backward compat for all 12 existing callers. Circular import avoided: scheme primitives (`detect_scheme`, `StorageScheme`, string helpers) live at the **top** of `path_utils.py`; `_get_backend()` does a lazy `from storage_backends import get_backend` **inside each dispatcher** at call time.
5. **Staging-swap atomic lives per-backend as a method** (`backend.staging_swap_atomic`) so per-scheme partition-overwrite semantics (POSIX leaf recursion vs S3 prefix-tuple inference) are encapsulated on the backend that owns the byte-plane, not distributed across `_staging_swap.py` conditionals. `_staging_swap.py` becomes a 99-line backward-compat shim that delegates to `storage_backends.atomic_swap`.
6. **Ingest-phase (L0→L1) independence preserved.** Because `path_utils` dispatchers resolve backends in pure Python (no Spark, no JVM), all INGEST work runs before any `SparkSession` is ever booted — satisfying the two-phase IO model (INGEST pure Python before Spark exists; NORMALIZE→SQL→PUBLISH runs Spark).
...
- **Support for schemes beyond `s3://` and POSIX local.** GCS, Azure Blob, HDFS native URI, ADLS Gen2, and any others are out of scope. The hard stop on unsupported schemes preserves correctness without artificially expanding scope. (They can be added later following the `StorageBackend` + `register_backend()` pattern implemented in B-6: add a new backend class, register it into `_BACKEND_REGISTRY` at import time, and add the new value to `StorageScheme` enum — zero changes to any call site.)
- **Dynamic plugin auto-discovery, entrypoint registration, or setuptools-based runtime plugging for new schemes.** Out of scope. The registry pattern is **static in-code registration**: either add the backend to the default `_BACKEND_REGISTRY` singleton at module load time in `storage_backends/__init__.py`, or explicitly call `register_backend(scheme, backend)` from a known bootstrap location. No scanning `sys.path`, no `pkg_resources.iter_entry_points`, no `importlib.metadata` plugins, no reflection over third-party packages, no HTTP/ConfigMap-based scheme discovery. Simplicity over extensibility.
