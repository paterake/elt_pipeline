...
| 5 | Pull in deferred `s3://` object-storage URI work now? | **Deferred then (at time of pathing backlog), NOW REINSTATED TO APPROVED SCOPE (2026-08-13):** Historical decision: *at the time of the pathing backlog* was to defer storage URI work to its own PRD (pathing was orthogonal). **Historical rationale was: P1–P5 changes are orthogonal to S3 URIs. Add path abstraction in layout classes (so `s3://` is a small later change) but do NOT gate pathing on storage backend. **Update 2026-08-13:** The deferral is now resolved. PRD 08 and the active TODO_STORAGE_URI backlog restore this work, implementing the full URI string + scheme dispatch convention from Mercell/Camelot, as the correct original design. The local-POSIX-only assumption is no longer authoritative. See active [TODO_STORAGE_URI.md](file:///Users/Rakesh.Patel/Documents/__code/git/emailrak/elt_pipeline/docs/todo/TODO_STORAGE_URI.md) + approved [PRD 08](file:///Users/Rakesh.Patel/Documents/__code/git/emailrak/elt_pipeline/docs/prd/08-prd-storage-root-uri-io-dispatch.md). |
