...
- Object storage URIs (`s3a://` etc.) for `level2+` — **NOTE (2026-08-13): DESCISION SUPERSEDED. SUBSEQUENTLY IMPLEMENTED AND COMPLETE.** This item was historically descoped for the Spark-only backlog; after completion of the path/partition management backlog, it was **reinstated to approved scope** as the correct original design per Mercell/Camelot conventions. Implementation is now **COMPLETE** via the separate storage-URI workstream:
  - Approved workstream tracked at: [TODO_STORAGE_URI.md](file:///Users/Rakesh.Patel/Documents/__code/git/emailrak/elt_pipeline/docs/todo/TODO_STORAGE_URI.md) (completion status archived in `docs/todo/archive/` when G5 passes).
  - Architecture spec: [PRD 08: URI-Aware Storage Root Paths and Explicit-Config I/O Dispatch](file:///Users/Rakesh.Patel/Documents/__code/git/emailrak/elt_pipeline/docs/prd/08-prd-storage-root-uri-io-dispatch.md)
  - What was implemented: string-typed root arguments in all CLI/runner/connector/Spark-executor signatures; `join_paths()` + scheme dispatcher in `src/elt_pipeline/shared/path_utils.py` replacing all `pathlib.Path /` joins on storage roots; native `s3://` handoff to `spark.read.parquet / writer.parquet`; fail-fast scheme guard `validate_config_root_schemes(...)` in `src/elt_pipeline/config/loader.py`; `pyproject.toml` extras `s3 = [boto3>=1.34]` and `emr = [s3+spark]`; EMR no-mounts execution pattern documented in `docs/operator/LOCAL_OPERATOR_RUNBOOK.md`.
  - The local-filesystem-only assumption is **no longer authoritative.** Use string URI roots + scheme dispatch per PRD 08 exclusively.
