...
- `examples/configs/local_sqlite_multi_table_simple.yaml`: **multi-table simple pattern** — one source block with shared `defaults.extraction` (connection, watermark, delta mode) cascades across 4 entities; most entities rely on auto `SELECT * FROM catalog_table`, two use declarative `filters:`, one uses `sql_file:` for custom joins, `{today.yyyymmdd}` placeholder for run-date filtering. Replaces the legacy redirect/lookup-probe pattern with flat, publishable YAML.
